...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'ilrestorante' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
...
function IsGradeBoss()
    if PlayerData ~= nil then
        ...
        if PlayerData.job.grade_name == 'dueño' then
            IsGradeBoss = true
        end
        return IsGradeBoss
    end
...
function setUniform(job, playerPed)
    TriggerEvent('skinchanger:getSkin', function(skin)
        ESX.ShowNotification(_U('0'))
        if skin.sex == 0 then
            if Config.Uniforms[job].male ~= nil then
                ESX.ShowNotification(_U('1'))
                TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms[job].male)
            else
                ESX.ShowNotification(_U('2'))
                ESX.ShowNotification(_U('no_outfit'))
            end
            if job ~= 'citizen_wear' and job ~= 'barman_outfit' then
                ESX.ShowNotification(_U('3'))
